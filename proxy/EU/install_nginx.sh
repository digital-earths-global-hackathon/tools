...
if [ -d /scratch/k/k202134 ] ; then 
    CACHE_DIR="/scratch/k/k202134/nginx"
else
...
if [ ! -x "$INSTALL_DIR/sbin/nginx" ]; then
    ...
    if [ ! -f pcre2-10.46.tar.bz2 ]; then
        wget https://github.com/PCRE2Project/pcre2/releases/download/pcre2-10.46/pcre2-10.46.tar.bz2
    ...
    ./configure \
        ...
        --with-pcre=$(pwd)/../pcre2-10.46 \
        --with-openssl=$(pwd)/../openssl-3.5.2
...
#    aio threads;
    ...
    proxy_cache_path $CACHE_DIR levels=1:2 keys_zone=my_cache:500m inactive=14d max_size=20000g use_temp_path=off;
